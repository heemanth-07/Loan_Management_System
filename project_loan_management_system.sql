...
MODIFY customer_id VARCHAR(55);

ALTER TABLE customer_income 
ADD primary key (customer_id);

-- SHEET 1 CUSTOMER_INCOME_STATUS


CREATE TABLE applicant_income(SELECT *,
...
SELECT * FROM applicant_income;


CREATE TABLE monthly_interest_percent(SELECT *,
		   CASE 
	       ...
	       ELSE "7" 
END AS monthly_interest_percent
FROM customer_income);

SELECT * FROM monthly_interest_percent;

-- TABLE 1

CREATE TABLE customer_income_status
(SELECT a.loan_id,a.customer_id,a.applicantincome,a.coapplicantincome,a.property_area,a.loan_status,a.applicant_income,
m.monthly_interest_percent
FROM applicant_income a INNER JOIN monthly_interest_percent m ON a.customer_id = m.customer_id);

SELECT * FROM customer_income_status;


-- SHEET 2 loan_status ( loan amount null and cibil score category )

-- primary table

CREATE TABLE loan_det
   (loan_id varchar(55),
   customer_id varchar(55),
   loan_amount varchar(55),
   loan_amount_term int,
   cibil_score int);

-- secondary table

CREATE TABLE loan_update
   (loan_id varchar(55),
   loan_amount varchar(55),
   cibil_score int,
   cibil_score_status varchar(55));
   
DELIMITER ^^
CREATE TRIGGER loan  BEFORE INSERT ON  loan_det FOR EACH ROW
BEGIN
...
DELIMITER ^^
CREATE TRIGGER loan_update_det  AFTER INSERT ON  loan_det FOR EACH ROW
BEGIN
      IF new.cibil_score > 900 THEN INSERT INTO loan_update(loan_id,loan_amount,cibil_score,cibil_score_status)
      values(new.loan_id,new.loan_amount,new.cibil_score,"high cibil score");
      
      ELSEIF new.cibil_score > 750 THEN INSERT INTO loan_update(loan_id,loan_amount,cibil_score,cibil_score_status)
      values(new.loan_id,new.loan_amount,new.cibil_score,"No penalty");
      
      ELSEIF new.cibil_score > 0 THEN INSERT INTO loan_update(loan_id,loan_amount,cibil_score,cibil_score_status)
...
SELECT * FROM loan_update;

-- loan_status table ( without loan still processing and reject customers )

CREATE TABLE loan_detail(
SELECT d.loan_id,d.customer_id,d.loan_amount,d.loan_amount_term,d.cibil_score,cibil_score_status 
FROM loan_det d INNER JOIN loan_update e ON 
d.loan_id = e.loan_id);

SELECT * FROM loan_detail;

DELETE FROM loan_detail WHERE loan_amount = "loan still processing" OR cibil_score_status = "reject customers";



-- loan_status table ( Monthly interest and annual interest )

CREATE TABLE customer_analysis(
SELECT c.loan_id,c.customer_id,c.applicantincome,c.coapplicantincome,c.property_area,c.loan_status,c.applicant_income,c.monthly_interest_percent,
l.loan_amount,l.cibil_score,l.cibil_score_status
FROM customer_income_status c 
INNER JOIN loan_detail l  ON c.loan_id = l.loan_id);
...
CREATE TABLE customer_interest_analysis(SELECT *,(loan_amount*monthly_interest_percent/100) AS  Monthly_interest_amount, 
round((loan_amount*monthly_interest_percent/100*12),2) AS Annual_interest_amount 
FROM customer_analysis);

SELECT * FROM customer_interest_analysis;

-- SHEET 3 customer_info ( update gender and age )

SELECT * FROM customer_det;

CREATE TABLE customer_info(SELECT  c.loan_id,c.customer_id,c.applicantincome,c.coapplicantincome,c.property_area,c.loan_status,c.applicant_income,c.monthly_interest_percent,
c.loan_amount,c.cibil_score,c.cibil_score_status,c.monthly_interest_amount,c.annual_interest_amount,d.gender,d.age
FROM customer_interest_analysis c INNER JOIN customer_det d  ON c.customer_id = d.customer_id);
...
-- joining all 5 tables  output 1

CREATE TABLE region(SELECT c.customer_id,c.loan_id,c.customer_name,c.region_id,c.postal_code,c.segment,c.state,r.region
FROM country_state c INNER JOIN region_info r ON c.region_id = r.region_id);

SELECT * FROM region;

CREATE TABLE customer_region_info(SELECT c.loan_id,c.customer_id,c.applicantincome,c.coapplicantincome,c.property_area,c.loan_status,c.applicant_income,c.monthly_interest_percent,
c.loan_amount,c.cibil_score,c.cibil_score_status,c.monthly_interest_amount,c.annual_interest_amount,c.gender,c.age,r.customer_name,r.region_id,r.postal_code,r.segment,r.state,r.region
FROM customer_info c INNER JOIN region r ON c.customer_id = r.customer_id);

SELECT * FROM customer_region_info;

-- mismatch details output 2

CREATE TABLE region_mismatch(SELECT c.customer_id,c.loan_id,c.customer_name,c.region_id,c.postal_code,c.segment,c.state,r.region
FROM country_state c RIGHT JOIN region_info r ON c.region_id = r.region_id);

SELECT * FROM region_mismatch;

delimiter %%
CREATE PROCEDURE emp_data ()
begin
SELECT * FROM customer_region_info WHERE cibil_score_status = "high cibil score";
SELECT * FROM customer_region_info WHERE segment = "corporate" or  segment = "home office";
end
%%
DELIMITER ;
...
-- COUNT 
SELECT COUNT(*) AS After_inserting_values FROM loan_det; 
SELECT COUNT(*) AS After_deleting_values FROM loan_detail; 
SELECT COUNT(*) AS segment_count FROM customer_region_info WHERE segment = "corporate" or  segment = "home office";
SELECT COUNT(*) AS high_cibil_score_count FROM customer_region_info WHERE cibil_score_status = "high cibil score";